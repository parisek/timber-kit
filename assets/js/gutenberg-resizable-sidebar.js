jQuery(window).ready(function () {
	setInterval(function () {
		jQuery(".interface-interface-skeleton__sidebar").width(
			localStorage.getItem("portadesign_gutenberg_sidebar_width")
		);
		jQuery(".interface-interface-skeleton__sidebar").resizable({
			...
			resize: function (event, ui) {
				...
				localStorage.setItem(
					"portadesign_gutenberg_sidebar_width",
					jQuery(this).width()
				);
			},
		});
	...
	function determine_if_sidebar_open() {
		...
		if (sidebar_enabled) {
			jQuery(".edit-post-layout, .edit-site-layout").addClass(
				"is-sidebar-opened"
			);
		} else {
			jQuery(".edit-post-layout, .edit-site-layout").removeClass(
				"is-sidebar-opened"
			);
		}
	}
