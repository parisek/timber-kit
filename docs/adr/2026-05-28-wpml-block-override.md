...
Matching is therefore by **block name + ordinal position**: the Nth occurrence of a name in the translation pairs with the Nth in the source. WPML/ATE rebuilds a translation from the source as a template, preserving block order and count, which makes the ordinal a reliable join key (verified: real pages carry duplicate same-named blocks, e.g. 2–3× `hero-text`, so name alone is insufficient). The per-name counter is reset at the start of each `the_content` pass (priority 5, before `do_blocks` at 9) so a secondary `do_blocks()` pass — e.g. an SEO plugin extracting a meta description — can't desync the ordinals the visible render relies on.

**Structural-integrity gate.** Position matching is only sound while the translation's block structure mirrors the source's. Before matching, `blockCountsMatch()` verifies the source and translation hold the *same number* of blocks of that name; on any difference (block added, removed, or manually reordered in a duplicate-then-edit translation) the whole block name is skipped — a safe no-op. This closes the one case raw position matching couldn't: equal-but-reordered structure landing a Copy value on the wrong instance. The blast radius without the gate was already bounded (a Copy value from a sibling block of the *same type*, only on pages with duplicate-named blocks *and* a manually divergent translation), and read-time-only with no DB writes; the gate removes it entirely at the cost of not overriding when structure has diverged.
