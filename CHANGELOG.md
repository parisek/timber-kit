...
- `Parisek\TimberKit\SvgDimensions` + `wp timber-kit svg-dimensions` + `StarterBase::$svg_dimensions` (default off) — give SVG attachments the intrinsic `width`/`height` WordPress never measures for them, so an `<img>` reserves its box instead of shifting the layout. `getimagesize()` cannot parse SVG, so core stores no dimensions at all; the `svg-support` plugin fills part of the gap but its reader takes only the `width`/`height` attributes on the root element, so a `viewBox`-only export — the current Figma and Illustrator default — stores `intval( '' )`, i.e. `0`. Measured on one production library: 1519 of 3520 SVGs unsized, and the share **growing** by upload year (2022: 555 of 1673; 2025: 353 of 458) as export tooling moves to `viewBox`-only. A 400-row sample of the unsized rows was 400/400 derivable from `viewBox`. The resolver reads the attribute pair first (an author's stated intent, and what a browser lays out), falls back to `viewBox`'s last two values, strips `px`, and rejects relative lengths like `width="100%"` — which carry no intrinsic size — rather than reading them as pixels. It parses **only the root start tag**, hand-scanned so an attribute value containing `>` cannot truncate it, and that is reliability rather than tidiness: parsing whole documents lost 15 of those 1519 to two failures a fixture would never have shown, both found by running against the real library. Three uploads hit libxml's 10 MB `AttValue` limit on an embedded `<image href="data:image/png;base64,…">`, which aborts the parse and takes the root's own `width`/`height` down with it even though they sit in the first hundred bytes; four were Illustrator exports whose root reads `xmlns:x="&ns_extend;"`, an entity the DOCTYPE declares. Undefined and bare ampersands are escaped to literal text rather than resolved — resolving a DOCTYPE entity is the XXE vector this shape exists to avoid, and no attribute that can carry one is ever read. With the root-only reader, all **1519/1519** resolve. Two rules make it safe beside any other plugin in this space: it writes core's own `_wp_attachment_metadata` `width`/`height` keys (the structure `wp_get_attachment_image_src()` reads, so the answer is shared rather than hidden in a private key), and it fills **only** a value that is missing or zero, so another plugin's answer always stands. A stored `0` counts as missing on purpose — it is a reader's failure, not a considered size. The upload filter hooks `wp_generate_attachment_metadata` at priority **20**, above the 10 that `svg-support` and friends use, so it observes their result instead of racing them by load order. The CLI sweep is registered unconditionally (a command a human types is opt-in by being typed) and covers the existing backlog the filter cannot reach; `--dry-run`, `--limit`, `--verbose`, and `--force` to re-derive over a value another plugin resolved wrongly. Deliberately a sweep rather than a lazy write during rendering: a page render must not write to the database, or the healing becomes a property of who requested an uncached page and stops on a read-only replica. No read-path change was needed — `Helpers::formatImageFrom()` already carries the dimensions through the moment they exist (verified against two attachments of one file: `meta=(60,26)` in, `(60,26)` out; `NULL` in, `NULL` out), so the core [#26256](https://core.trac.wordpress.org/ticket/26256) 1px guard is not the defect, it simply had nothing to read. Off by default per the flag doctrine: an `<img>` that carried no dimensions starts carrying them, which is the point, but it is still a change to markup a project may have laid out around.
