...
- `Helpers::getEditorAllowedHtml()` now permits `aria-label` on `<a>`, and
  ...
  reverse-tabnabbing route is closed in the engine too.

  **Why it matters in practice.** This list runs on `acf/update_value/type=wysiwyg`,
  ...
  being stripped starts being stored except these two attributes on anchors.
  Note the direction is the safe one by construction: because the list runs at
  save time, **removing** an entry would delete stored content on a field's
  next save, while adding one cannot.
