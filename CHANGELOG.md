...
- `Helpers::formatFields()` now resolves ACF field groups in two contexts where ACF's default `get_field_objects()` location matcher silently drops registered groups:
  ...
  Known limitation tracked for follow-up: projects mixing default-namespace and **custom-namespace** options pages (`acf_add_options_page(['post_id' => 'company_settings'])`) see a cross-namespace union in this release — `formatFields('option')` lists field definitions from custom-namespace pages and then `get_field()` returns `null` for their values (silently dropped by `fieldFormatter()`'s empty-value filter). The matching namespace filter ships in a follow-up PR. Single-namespace projects are unaffected.
