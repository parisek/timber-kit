...
- **`StarterBase::$options_pages` property (array)** — declarative config for the ACF options page(s), replacing the previously hard-coded single "Theme Settings" page. Each entry is one page: `menu_slug` + `page_title` are required; `parent_slug` makes it a sub-page (`acf_add_options_sub_page`) under that parent; `graphql_field_name` is otherwise derived GraphQL-safe from the slug (all non-alphanumerics → `_`, so multiple pages don't collide and the default `settings` slug keeps the released `settings` field name). Defaults to a single "Theme Settings" page (no behavioural change); the admin-bar "Theme Settings" link targets the first top-level page (skipped if none). Top-level pages are registered before sub-pages, so list order doesn't matter, and `acf_add_options_sub_page()` is `function_exists`-guarded. Set `$options_pages = []` to register no options pages at all — completely disables the feature (no ACF page, no admin-bar link).
