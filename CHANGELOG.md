...
- **`WpmlBlockOverride` — render-time Copy-field sync for ACF Gutenberg blocks under WPML/ACFML** ([#29](https://github.com/parisek/timber-kit/issues/29)). Opt-in via `\Parisek\TimberKit\WpmlBlockOverride::register()`. Hooks `render_block_data` at priority 20 (after WPML's own handlers) and, for ACF blocks rendered in a non-default language, overwrites each Copy field's `attrs.data.<field>` with the value from the **source-language** post — at render time, no DB writes. Fixes the long-standing WPML gap (since ACFML 1.3, 2019) where changing a Copy field (typically an image) in the source language never propagates to translations: a Copy change isn't a *translatable* change, so WPML never marks the translation `needs_update` and the stale value ships until someone manually re-runs the ATE job. ACF config becomes the single source of truth for Copy fields; the Translate flow stays entirely with ACFML/ATE. Copy fields are auto-detected from the block's field groups (ACFML "Copy" preference; `_`-prefixed system fields skipped; cached 24h, flushed on `acf/update_field_group`) or declared explicitly via the `timber_kit/wpml_block_override/copy_fields` filter; numeric attachment values are remapped to the current language's duplicated media via `wpml_object_id`. Bypassed in admin/REST (editor-preview safety) and safe-degrades to the stored value on structural drift or a missing block id. Nested Repeater/Group/Flexible Copy fields are out of MVP scope.
