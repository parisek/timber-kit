...
- `BlockRenderer::registerInvalidation()` — registers the `acf/save_post` cache invalidation hook at priority 20 (flushes cache group `acf_block_{$post_id}` when the external object cache supports `flush_group`). Migrated from the freestanding `add_action` in the original `functions.php`; now lives alongside the cache writer so the two can't drift.
- `StarterBase::register_timber_kit_namespace()` — registers the `@timber-kit/` Twig namespace pointing at `src/templates/` via the `timber/locations` filter at priority 5, so downstream themes can override individual templates at the default priority 10.

### Changed
- `StarterBase::__construct()` — adds the `timber/locations` filter registration for `register_timber_kit_namespace()` and the `BlockRenderer::registerInvalidation()` boot call alongside the existing `timber/*` filter registrations.
