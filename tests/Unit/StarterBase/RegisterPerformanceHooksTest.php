...
class RegisterPerformanceHooksTest extends StarterBaseTestCase {
	...
	public function test_resizer_quality_in_cache_key_no_override_by_default(): void {
		...
		$this->setProperty( $instance, 'warn_speculation_rules_plugin_redundant', false );
		$this->setProperty( $instance, 'resizer_format_health', false );
		$this->setProperty( $instance, 'resizer_quality_in_cache_key', false );

		$this->invokeRegisterPerformanceHooks( $instance );

		// Default off → historic cache paths, so nothing is wired.
		$this->assertNotContains( 'timber_kit_resizer_quality_in_cache_key', $filters );
	}

	public function test_resizer_quality_in_cache_key_override_registered_when_enabled(): void {
		$filters = [];
		Functions\when( 'add_filter' )->alias( function ( $hook, ...$rest ) use ( &$filters ) {
			$filters[] = $hook;
		} );
		...
		$this->setProperty( $instance, 'warn_speculation_rules_plugin_redundant', false );
		$this->setProperty( $instance, 'resizer_format_health', false );
		$this->setProperty( $instance, 'resizer_quality_in_cache_key', true );
