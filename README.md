...
| `$preload_headers` | bool | `true` | Also send the preload hints as a `Link:` response header, alongside the `<link rel=preload>` tags core renders from the same list. The header arrives before the body, so every browser acts on it earlier; it is also the whole of an origin's part in HTTP 103 Early Hints, which an edge synthesises from `Link:` headers it saw on a previous response — PHP cannot emit an informational response itself. Runtime override: `timber_kit_preload_headers`. Whatever a project adds to `wp_preload_resources` must be public and identical for every visitor: an edge stores these headers per URL and replays them to whoever asks next |
| `$preconnect_origins` | array | `[]` | Absolute origins advertised with `rel=preconnect` in that header (e.g. a tag manager, a font host). Paths are stripped — a preconnect opens a connection, it fetches nothing. Runtime override: `timber_kit_preconnect_origins` |
...
| `$seo_suppress_plugin_breadcrumb` | bool | `true` | Keep one `BreadcrumbList` per page, and make it the theme's. Every theme here renders its breadcrumb as microdata from `timber_kit_breadcrumb_items`; the SEO plugin adds a second one to its JSON-LD graph from data it derived itself, and a crawler picks. The plugin's is the worse copy because it cannot see the trail the theme decided on — measured on two projects, where the page showed `Úvod > …` while the graph said `Home > …`. **Defers to the plugin's own "Enable Breadcrumbs" switch where it exists:** turn that on and this stands aside. AIOSEO keeps that switch only on sites that had breadcrumbs off before it deprecated the setting, and it is on a removal list, so an absent switch counts as no signal rather than as off. Neither plugin can be told to drop only the schema in its own settings. Set `false` to keep both |
| `$disable_author_archives` | bool | `true` | Turn `/author/{nicename}/` archives into a 404 on `template_redirect`, at the same priority as `$block_author_enumeration`. That flag blocks `/?author=N` because core redirects it to this URL and the redirect leaks the login slug — but the URL it redirects to kept answering, so the guard was only half of one. Implies `$disable_author_sitemap`: an archive that answers 404 must not be listed anywhere. The 404 is set on the request rather than by dropping the rewrite rules, which are cached in the database and would leave the flag looking inert until someone ran `wp rewrite flush`. Set `false` on a site that deliberately publishes author pages |
...
Each file is encoded to a temp file in its own directory and renamed over the target. The rename is atomic on one filesystem, so the URL is never missing and never serves half a file. A new file replaces the old one only once it encoded, weighs something and decodes; any other outcome deletes the temp file, keeps the old one and reports `failed`. Nothing is ever deleted: a derivative whose source file is gone is reported as an orphan and left in place, which is `clear-image-cache`'s job.
...
**A big site spreads the cost over several nights.** `--older-than` is the resume mechanism: a regenerated file carries a fresh mtime, so the next run with the same cutoff skips what the last one finished. Pick the moment the sweep started and keep it for the whole sweep. `--limit` caps one night's batch, `--sleep=<ms>` puts a gap between files, and `--max-load=<n>` holds before each file while the one-minute load average is above `n`, giving up after 30 minutes and reporting how many files remain.

```cron
# Re-encode up to 2000 AVIF derivatives a night, gently, until none is left.
0 2 * * * cd /var/www/html && wp timber-kit regenerate-image-cache --format=avif --older-than='2026-09-16 00:00' --limit=2000 --sleep=150 --max-load=4 --apply >> /var/log/regen.log 2>&1
```
...
- `timber_kit_resizer_target_quality` — encoder quality 0-100, default `80` (was `100` before 1.53.0). Honoured for AVIF since 1.53.0 (earlier, except on the smart-crop path, on sites whose lock already had spatie/image >= 3.9.6): before that the AVIF encoder ran at its own default under `100`, and at `100 - quality` under any other value. A site upgrading keeps its old AVIF files until `wp-content/cache/image/` is cleared. A site that sets `100` gets true quality 100, about 25x the old size for an AVIF photo.
- `timber_kit_resizer_quality_in_cache_key` — put a variant's quality in its cache key, default `false` (also settable as `StarterBase::$resizer_quality_in_cache_key`). Without it, re-cutting the same dimensions at a different quality serves the previously generated file. Quality 100 stays out of the path; every other quality, the default 80 included, gets a `-q<N>` suffix. Opt-in because switching it on relocates every variant whose quality is not 100: old cache files orphan and public URLs change.
- `timber_kit_resizer_source_path_in_cache_key` — key the resizer cache on the source's whole identity, default `false` (also settable as `StarterBase::$resizer_source_path_in_cache_key`): its upload directory *and* its own filename, extension included. Without it, two uploads that share a directory-less, extension-less stem occupy one cache path and whichever renders first decides what the other one shows — that covers both `2022/03/11.png` vs. `2022/10/11.png` (directory) and `hero.jpg` vs. `hero.png` in one directory (extension). Opt-in because switching it on relocates **every** derivative — a source at the uploads root included, since the name carries the source's own extension — so old cache files orphan and public URLs change. The name is the stored one, used verbatim rather than re-sanitized (ADR 0008), so a derivative's URL percent-encodes each path segment. Note that on a case-insensitive filesystem `Ebook.svg.avif` and `ebook.svg.avif` remain one file, so a macOS dev box and a Linux server disagree about such a pair. **Set it the same on every host that shares a cache.** `TIMBERKIT_MEDIA_ORIGIN` (DevMediaProxy) has a local site request derivative paths from a production origin; with the flag on locally and off in production, or the reverse, every one of those paths is a 404 on the other host until both agree. Run `wp timber-kit migrate-image-cache` to move them instead of re-encoding them. The command reports with the flag off — reading the plan before deciding is the point — but refuses `--apply` until the flag is on, since nothing would read the layout it moves files into — a flat legacy derivative whose old name maps to more than one distinct source is reported as ambiguous and left in place, since recovering which source it came from is exactly what the flat layout destroyed.
- `timber_kit_resizer_cache_version` — site-wide version appended to every derivative URL as `?v=<version>`, default empty (also settable as `StarterBase::$resizer_cache_version`). Set or bump it to make browsers, the proxy and a CDN fetch derivatives again, for example after they were regenerated at the same path. It changes URLs only: files on disk are not regenerated (delete them with `wp timber-kit clear-image-cache`), a CDN that ignores query strings does not see it, and the page cache must be purged so HTML carries the new URLs. The kit's own block cache includes the version in its key, so cached blocks miss on a bump. Empty leaves URLs byte-identical. DevMediaProxy appends the same version to remote variant URLs. `Resizer::appendCacheVersion( $url, $version )` exposes the same rule.
...
> **Upgrading with `$mce_exclude_editor_styles` — check the file once.** Search the theme's `static/src/css/gutenberg-editor.css` for `mce-content-body` or `#tinymce`. A match means the project deliberately styles the classic editor from that file, and excluding the file turns those rules off. Either drop the block — if it only restored what Preflight had broken, the native editor now does that job — or set `$mce_exclude_editor_styles = false` and keep it. Nothing warns about this: both states render, one just looks like the native editor and the other like the project's.
>
> The exclusion is also not "nothing is lost". A theme's own `settings.css` imports reach the iframe, so the compiled file typically also carries `:root` custom properties, base-layer element rules (`button { cursor: pointer }`, `html { overflow-anchor: none }`) and class-scoped block styles (`.wp-block-image`, `.wp-block-separator`). None of that styles the text a `wysiwyg` field holds — which is why the flag is safe — but a project with different imports should read its own compiled output rather than trust that list.
