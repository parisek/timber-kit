...
| `$disable_comments` | bool | `true` | Disable comments site-wide: removes comments/trackbacks support from every registered post type and from any post type registered later (via `registered_post_type`), closes `comments_open`/`pings_open`, strips comment admin pages and admin-bar/dashboard widgets, removes `/wp/v2/comments` REST routes, rejects REST comment insertion with 403 even if a route is re-registered, removes comment + pingback XML-RPC methods, drops the `X-Pingback` header, and forces `default_comment_status`/`default_ping_status` to `closed` |
