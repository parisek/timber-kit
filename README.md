...
- `timber_kit_resizer_target_quality` — encoder quality 0-100. Unset, AVIF gets `80` and every other format `100`; set, the value applies to every format. Honoured for AVIF since 1.53.0 (earlier, except on the smart-crop path, on sites whose lock already had spatie/image >= 3.9.6): before that the AVIF encoder ran at its own default when no quality was set, and at `100 - quality` when one was (spatie/image < 3.9.6, and the smart-crop path). A site upgrading keeps its old AVIF files until `wp-content/cache/image/` is cleared. A site that sets `100` explicitly gets true quality 100, about 25x the old size for a photo.
