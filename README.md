...
- `timber_kit_resizer_target_quality` — encoder quality 0-100, default `100`. Honoured for AVIF since 1.53.0: before that the AVIF encoder ran at its own default under the default `100`, and at `100 - quality` under any other value (spatie/image < 3.9.6, and the smart-crop path). A site upgrading from an earlier version keeps its old AVIF files until `wp-content/cache/image/` is cleared; after that, AVIF sizes follow the quality actually set, which under the default `100` is about 25x larger (measured: 18 KB to 454 KB for one 1600 px photo). Set an explicit value.
